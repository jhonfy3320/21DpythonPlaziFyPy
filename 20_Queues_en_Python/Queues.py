...
'''
Un Queue (cola) es una estructura de datos lineal que sigue la regla de “primero en entrar, primero en salir” (FIFO). 
En otras palabras, el primer elemento que se inserta en la cola es el primero en ser eliminado. 
Funciona como una fila de elementos en la que nuevos elementos se agregan al final y se eliminan del frente. 
Las colas son útiles en situaciones donde es importante mantener el orden de llegada de los elementos.'''

## Complejidad algorítmica

'''
Los métodos en una cola tienen una complejidad algorítmica constante, lo que significa que la eficiencia no depende del tamaño de la cola. 
Veamos la complejidad de los principales métodos:
'''
...
'''
Procesamiento en el mismo orden de llegada: 
Cuando necesitas procesar datos en el mismo orden en que fueron recibidos, como mensajes de correo electrónico, solicitudes de clientes, 
tareas en un sistema de planificación, etc.

Registro de eventos: 
Para mantener un registro de los últimos elementos agregados o modificados, como los registros de transacciones de un sistema de base de datos.

Recorrido en amplitud (BFS): 
En algoritmos de búsqueda en árboles o grafos, como el recorrido en amplitud (BFS), donde se deben explorar los nodos en orden de 
proximidad desde un nodo inicial.
'''
...
Acceso aleatorio: 
Si necesitas acceso aleatorio a los elementos (por índice), es más eficiente utilizar estructuras como arrays o listas enlazadas.

Búsqueda de elementos específicos: 
Si necesitas buscar elementos específicos dentro de la cola, también sería más eficiente usar una estructura de datos como un hash table (diccionario) para obtener una búsqueda más rápida.
Construyamos un Queue

A continuación, mostraremos una implementación de una cola en Python utilizando la clase Node para representar los nodos del queue y la clase 
Queue para definir la estructura de la cola:
'''
class Node:
